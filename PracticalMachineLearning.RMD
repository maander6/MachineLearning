...
### Random Forest
 ...
 A random forest model was next applied to the dataset to see if it would lead to an improvement in prediction accuracy.
```{r Random_Forest}
set.seed(5678)
ptm <- proc.time()
rf_mod <- randomForest(as.factor(classe) ~ ., data=training, ntree=1000)
tm1 <- proc.time()-ptm
## 
...
##
rf_model2 <- randomForest(as.factor(classe) ~ ., data=training1, ntree=1000)
tm2 <- proc.time() - ptm
tm2
